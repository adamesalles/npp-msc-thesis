...
% ---
\titulo{Exact Computation for\\ the Normalised Power Prior}
\autor{Eduardo Adame}
...
% Capa
% ---
% {\LARGE \color{red} PRELIMINARY VERSION ({\selectlanguage{british}\today})}
\imprimircapa
% ---
\cleardoublepage
...
\begin{epigrafe}
    \vspace*{\fill}
		\begin{flushright}
            \noindent
            \textit{``Se eu soubesse antes o que sei agora, erraria tudo exatamente igual.''}\\
			{\tiny \itshape ``If I knew then what I know now, I would have made exactly the same mistakes.''}

            From \emph{Surfando Karmas \& DNA} (2001) by Engenheiros do Hawaii
		\end{flushright}
	    \begin{flushright}
            \noindent
            \textit{``So, nobody told me what to do, and there was no preconception of what to do.''}\\
            From \emph{Giorgio By Moroder} (2013) by Daft Punk
		\end{flushright}
\end{epigrafe}
...
\input{chapters/03_approximate}

\input{chapters/04_exact}
...
\input{appendices/formulations.tex}
% ----------------------------------------------------------
% \input{src/appendice}
