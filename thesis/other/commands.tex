
\renewcommand{\phi}{\varphi}
\renewcommand{\epsilon}{\varepsilon}

\newcommand{\ds}{\displaystyle}
\newcommand{\eps}{\varepsilon}
\newcommand{\ssty}{\scriptstyle}
\newcommand{\bE}{\mathds{E}}
\newcommand{\bV}{\mathbb{V}}
\newcommand{\cB}{\mathcal{B}}
\newcommand{\cX}{\mathcal{X}}
\newcommand{\cY}{\mathcal{Y}}
\newcommand{\cU}{\mathcal{U}}
\newcommand{\cF}{\mathcal{F}}
\newcommand{\cA}{\mathcal{A}}
\newcommand{\cM}{\mathcal{M}}
\newcommand{\cD}{\mathcal{D}}
\newcommand{\cP}{\mathcal{P}}
\newcommand{\cN}{\mathcal{N}}
\newcommand{\cL}{\mathcal{L}}
\newcommand{\cLN}{\mathcal{LN}}
\newcommand{\bP}{\mathbb{P}}
\newcommand{\bQ}{\mathbb{Q}}
\newcommand{\bN}{\mathbb{N}}
\newcommand{\bR}{\mathds{R}}
\newcommand{\bZ}{\mathbb{Z}}
\newcommand{\bC}{\mathbb{C}}

% \renewcommand{\boldsymbol}{\symbf}

\newcommand{\bfP}{\boldsymbol{P}}
\newcommand{\bfQ}{\boldsymbol{Q}}
\newcommand{\bfX}{\boldsymbol{X}}
\newcommand{\bfY}{\boldsymbol{Y}}
\newcommand{\bfZ}{\boldsymbol{Z}}
\newcommand{\bfM}{\boldsymbol{M}}
\newcommand{\bfU}{\boldsymbol{U}}

\newcommand{\bfz}{\boldsymbol{z}}
\newcommand{\bfm}{\boldsymbol{m}}
\newcommand{\bfw}{\boldsymbol{w}}
\newcommand{\bfv}{\boldsymbol{v}}
\newcommand{\bfs}{\boldsymbol{s}}
\newcommand{\bfu}{\boldsymbol{u}}
\newcommand{\bfx}{\boldsymbol{x}}
\newcommand{\bfy}{\boldsymbol{y}}
\newcommand{\bff}{\boldsymbol{f}}
\newcommand{\bfb}{\boldsymbol{b}}
\newcommand{\bfa}{\boldsymbol{a}}
\newcommand{\bfth}{\boldsymbol{\theta}}
\newcommand{\bfmu}{\boldsymbol{\mu}}
\newcommand{\bfxi}{\boldsymbol{\xi}}
\newcommand{\bfsg}{\boldsymbol{\sigma}}
\newcommand{\bfeps}{\boldsymbol{\varepsilon}}

\newcommand{\poi}{\text{Poisson}}
% \newcommand{\var}{\text{Var}}
% \DeclareMathOperator{\cov}{Cov}
\newcommand{\gama}{\text{Gama}}
\newcommand{\normal}{\text{Normal}}
\newcommand{\ig}{\text{Inverse-Gamma}}
\DeclareMathOperator{\InvGam}{Inv-Gamma}
\newcommand{\ber}{\text{Bernoulli}}
\newcommand{\jg}{\text{Jg}}
\newcommand{\st}{\text{ s.t. }}
\newcommand{\otw}{\text{ otherwise }}
\newcommand{\sge}{\sigma_e^2}
\newcommand{\sgt}{\sigma_\theta^2}
\newcommand{\byi}{\bar{y}_i}

\newcommand{\dU}{\mathcal{U}}

\newcommand{\bX}{\boldsymbol{X}} %%  random sample, contracted form (bold)
\newcommand{\bx}{\boldsymbol{x}} %%  random sample, realised, contracted form (bold)
\newcommand{\bfp}{\boldsymbol{p}}
\newcommand{\bfbt}{\boldsymbol{\beta}}
\newcommand{\bfeta}{\boldsymbol{\eta}}
\newcommand{\bfgm}{\boldsymbol{\gamma}}
\newcommand{\bfV}{\boldsymbol{V}}

\newcommand{\gp}{\mathcal{GP}}
\newcommand{\tbx}{\tilde{\bfx}}
\newcommand{\tbX}{\tilde{\bfX}}
\newcommand{\tby}{\tilde{\bfy}}
\newcommand{\tbY}{\tilde{\bfY}}
\newcommand{\tbf}{\tilde{\bff}}
\newcommand{\yst}{\boldsymbol{y_\star}}
\newcommand{\Yst}{\boldsymbol{Y_\star}}
\newcommand{\fst}{\boldsymbol{f_\star}}
\newcommand{\xst}{\boldsymbol{x_\star}}
\newcommand{\Xst}{\boldsymbol{X_\star}}

\newcommand{\pr}{\mathds{P}}
\newcommand{\vr}{\operatorname{\mathds{V}}} %% 
\newcommand{\im}{\text{Im}}

\renewcommand{\epsilon}{\varepsilon}

\newcommand{\bvecc}[2]{%
  \begin{bmatrix} #1 \\ #2  \end{bmatrix}
}
\newcommand{\bveccc}[3]{%
  \begin{bmatrix} #1 \\ #2 \\ #3  \end{bmatrix}
}

\newtheorem{lemma}{Lemma}[section]
\newtheorem{definition}{Definition}[section]
\newtheorem{property}{Propriedade}[section] % eu que criei
\newtheorem{corollary}{Corollary}[section]
\newtheorem{teorema}{Teorema}[section]
\newtheorem{example}{Exemplo}[section]
\newtheorem{lema}{Lema}[section]
\newtheorem{prop}{Proposi\c{c}\~ao}[section]
\newtheorem{defi}{Defini\c{c}\~ao}[section]
\newtheorem{obs}{Observa\c{c}\~ao}[section]

\DeclareMathOperator*{\argmax}{arg\,max}
\DeclareMathOperator*{\argmin}{arg\,min}
\DeclareMathOperator*{\logit}{logit}

\newcommand{\indep}{\perp \!\!\! \perp} 

\newcommand{\comLM}[1]{\ifcomments\textcolor{red}{\texttt{Luiz:} #1}\fi}
\newcommand{\comFB}[1]{\ifcomments\textcolor{red}{\texttt{Flávio:} #1}\fi}
\newcommand{\comEA}[1]{\ifcomments\textcolor{magenta}{\texttt{Adame:} #1}\fi}

\newcommand{\hf}{\hat{f}}
\newcommand{\htheta}{\hat{\theta}}
\newcommand{\bfSigma}{\mathbf{\Sigma}}

\renewenvironment{proof}{%
\noindent\emph{Proof.}
}{%
\qed
}

% \floatname{algorithm}{Algoritimo}
% \renewcommand{\algorithmicif}{\textbf{se}}%
% \renewcommand{\algorithmicthen}{\textbf{então}}%
% \renewcommand{\algorithmicfor}{\textbf{para}}%
% \renewcommand{\algorithmicdo}{\textbf{faça}}%
% \renewcommand{\algorithmicend}{\textbf{fim}}%

\newcommand{\subf}[2]{%
  {\small\begin{tabular}[t]{@{}c@{}}
  #1\\#2
  \end{tabular}}%
}

\newcommand{\froyo}[1]{\color{red}#1}

\newcommand{\dbar}{\bar{d_0}_p}
\newcommand{\sg}{\sigma^2}
\newcommand{\sz}{\sigma_0^2}