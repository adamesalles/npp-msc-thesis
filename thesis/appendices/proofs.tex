...
To confirm that marginalising $\widetilde\pi(\eta, \theta^\star)$ over $\theta^\star$ recovers
  the desired target, note that
  \begin{align}
      \int \widetilde\pi(\eta, \theta^\star) \, \dd\theta^\star
      &\propto
      \pi(\eta \mid \theta) \int \pi_{\eta'}(\theta^\star \mid D_0) \, \dd\theta^\star
      =
      \pi(\eta \mid \theta),
  \end{align}
  since $\pi_{\eta'}(\cdot \mid D_0)$ is a proper density.
As the augmented kernel satisfies detailed balance with respect to $\widetilde\pi$, and since the marginal of $\widetilde\pi$ over $\theta^\star$ recovers $\pi(\eta \mid \theta)$, the exchange update has the correct invariant distribution.
\end{proof}
